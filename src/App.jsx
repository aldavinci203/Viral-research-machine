...
const TIKTOK_ALGO = `RANKING: Completion rate 35% (most important), Rewatches 20%, Shares 18% (1 share = 3 likes), Comments 12%, Likes 8%, Saves 7%. HOW TESTING WORKS: TikTok first pushes to 300-500 viewers. Good completion = push to 5K, then 50K, then viral. 300 views = failed the first completion test. Hook window: 0.5s swipe decision, 3s commit decision. Optimal length: 7-15s for max completion. WHAT KILLS GOOD VIDEOS: wrong posting time, no engagement in first 30 mins, slow hook payoff, dead hashtags, account momentum issues. POSTING: Best Tue-Fri 7-9am or 7-11pm. HOOKS: Numbers increase CTR 36%, questions increase completion 28%, You > I statements. Trending audio = 2.1x reach, original = 1.4x reach. Questions in captions increase comments 40%.`;
...
Analyze with surgical precision. Return ONLY valid JSON in \`\`\`json blocks:
{
  ...
  const askVideoQuestion=async()=>{
    if(!predQuestion.trim()||!predResult||chatLoading)return;
    const question=predQuestion.trim();
    setPredQuestion("");
    setChatLoading(true);
    const newChat=[...predChat,{role:"user",text:question}];
    setPredChat(newChat);
    try{
      const context=predResult.raw
        ?predResult.raw
        :JSON.stringify(predResult);
      const answer=await geminiText(gKey,
        `You are a TikTok expert who just analyzed a creator's video. Here is the full analysis:

${context}

TIKTOK ALGORITHM CONTEXT:
- Completion rate is the #1 signal (35% weight)
- TikTok tests videos with a small audience first (300-1000 views). If completion rate is good, it pushes wider
- 300 views often means TikTok tested it but completion rate wasn't strong enough to push further
- Posting time matters — bad timing can kill a great video
- Account momentum matters — if recent videos underperformed, algorithm is skeptical
- Sometimes great videos just don't get pushed — it's not always about quality

CREATOR CONTEXT: ${CREATOR_DNA}

The creator is asking: "${question}"

Answer directly, honestly, and specifically based on the video analysis. If they're asking why a video didn't perform, be real about what the data shows vs what might be external factors (timing, momentum, algorithm randomness). Give actionable advice. Be conversational not corporate.`
      );
      setPredChat(prev=>[...prev,{role:"assistant",text:answer}]);
    }catch(e){
      setPredChat(prev=>[...prev,{role:"assistant",text:"Error: "+e.message}]);
    }
      ...
      <div style={{maxWidth:680,margin:"0 auto",padding:"18px 16px"}}>
        ...
        {tab==="predict"&&(
          <div>
            ...
            {predResult&&(
              <Card glow>
                <Lbl color={C.accent}>💬 Ask About Your Video</Lbl>
                <div style={{fontSize:12,color:C.muted,marginBottom:13,lineHeight:1.6}}>Ask anything — "Why did this only get 300 views?", "Should I repost this?", "What would change the hook?", "Is this concept worth trying again?"</div>
                
                {/* Suggested questions */}
                <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:13}}>
                  {["Why did this underperform?","Should I repost this?","What's the #1 fix?","Is the concept worth trying again?","When should I post this?"].map(q=>(
                    <button key={q} onClick={()=>setPredQuestion(q)} style={{padding:"5px 10px",borderRadius:8,border:`1px solid ${C.accent}44`,background:C.accent+"11",color:C.accent,fontSize:11,cursor:"pointer",fontWeight:500}}>{q}</button>
                  ))}
                </div>

                {/* Chat history */}
                {predChat.length>0&&(
                  <div style={{marginBottom:13,maxHeight:400,overflowY:"auto"}}>
                    {predChat.map((m,i)=>(
                      <div key={i} style={{marginBottom:11,display:"flex",gap:9,alignItems:"flex-start",flexDirection:m.role==="user"?"row-reverse":"row"}}>
                        <div style={{width:28,height:28,borderRadius:8,background:m.role==="user"?C.accent:C.green,display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,flexShrink:0}}>
                          {m.role==="user"?"👤":"🧠"}
                        </div>
                        <div style={{background:m.role==="user"?C.accent+"22":C.surface,borderRadius:10,padding:"10px 13px",maxWidth:"80%",fontSize:13,color:"#C8CAE0",lineHeight:1.6,border:`1px solid ${m.role==="user"?C.accent+"44":C.border}`}}>
                          {m.text}
                        </div>
                      </div>
                    ))}
                    {chatLoading&&(
                      <div style={{display:"flex",gap:9,alignItems:"center"}}>
                        <div style={{width:28,height:28,borderRadius:8,background:C.green,display:"flex",alignItems:"center",justifyContent:"center",fontSize:12}}>🧠</div>
                        <div style={{background:C.surface,borderRadius:10,padding:"10px 13px",fontSize:13,color:C.muted}}>Thinking...</div>
                      </div>
                    )}
                  </div>
                )}
                ...
                <div style={{display:"flex",gap:8}}>
                  <input
                    ...
                    onKeyDown={e=>e.key==="Enter"&&askVideoQuestion()}
                    style={{flex:1,background:"#090B16",border:`1px solid ${C.border}`,borderRadius:10,padding:"11px 13px",color:C.text,fontSize:13,outline:"none",fontFamily:"inherit"}}
                  />
                  <Btn onClick={askVideoQuestion} disabled={chatLoading||!predQuestion.trim()} color={C.accent}>Ask</Btn>
                </div>
              </Card>
            )}
          </div>
